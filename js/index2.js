...
for (let i=0;i<sortBtn.length;i++){
    ...
    sortBtn[i].onclick=function () {
        ...
        //up和down跟随
        arrowFollow.call(this);//将函数中的this变成你点击的a标签
        clearOther.call(this);
    }
}
function sort(sortAttr) {
    data.sort((a,b)=>{
        return a=sortAttr=="time"?(new Date(a[sortAttr])-new Date(b[sortAttr]))*this.flg:(a[sortAttr]-b[sortAttr])*this.flg;
    });
    bindData(data);
}
function arrowFollow() {
    //根据点击的a标签(this)将下面的俩三角(up和down)实现对应
    //获取当前点击的a标签中的两个i  就是获取它的俩孩子
...
function clearOther() {
    //只要this(不是当前点击的a)标签 让里面的i标签没有"bg"类名
    for (let i=0;i<sortBtn.length;i++){
        if (sortBtn[i]!=this){
